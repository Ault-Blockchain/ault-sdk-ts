...
Installing from the npm registry is the supported path for production use.
...
Releases are automated by GitHub Actions on version tags.
...
3. GitHub Actions publishes to npm and creates a GitHub release for the tag.

Repository setup required for publishing:

- `NPM_TOKEN` GitHub Actions secret with publish rights for `ault-sdk-ts`.
