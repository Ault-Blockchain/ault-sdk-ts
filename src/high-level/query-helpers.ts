...
export function createParallelQueryApi(lowLevel: AultClient): ParallelQueryApi {
    ...
    for (let i = 0; i < indices.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.all(
        batch.map(async (index) => {
          try {
            const result = await lowLevel.rest.license.getTokenOfOwnerByIndex(
              normalizedOwner,
              index,
            );
            return result.id;
  ...
  async function getLicenseDelegationsParallel(
    licenseIds: string[],
  ): Promise<LicenseDelegationStatus[]> {
  ...
  async function analyzeLicenses(owner: string): Promise<LicenseAnalysis> {
    ...
    const active = validLicenses.filter(
      (license) => license.status === "LICENSE_STATUS_ACTIVE",
    ).length;
