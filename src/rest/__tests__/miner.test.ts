...
import { createMockFetch, mockJsonResponse, mockErrorResponse, type MockFetch } from "./mock-fetch";
...
describe("MinerApi", () => {
  ...
  describe("getEpochInfo", () => {
    it("fetches epoch info", async () => {
      mockFetch = createMockFetch(mockJsonResponse({ info: sampleEpoch }));
      context.fetchFn = mockFetch;
      api = createMinerApi(context);
  ...
  describe("getEpochs", () => {
    it("fetches epochs list without params", async () => {
      mockFetch = createMockFetch(mockJsonResponse({ epochs: [sampleEpoch] }));
      context.fetchFn = mockFetch;
      api = createMinerApi(context);
  ...
  describe("getLicensePayouts", () => {
    it("fetches license payouts without params", async () => {
      mockFetch = createMockFetch(mockJsonResponse({ total_payout: "1000", total_credits: "25" }));
      context.fetchFn = mockFetch;
      api = createMinerApi(context);
