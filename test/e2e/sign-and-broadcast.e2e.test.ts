...
const shouldBroadcast =
  shouldRun &&
  (process.env.AULT_TEST_BROADCAST === "1" || process.env.AULT_TEST_BROADCAST === "true");
...
async function waitForNewLicenseId(
  client: AultClient,
  owner: string,
...
describe("E2E Configuration Check", () => {
  it("should show test configuration status", () => {
    ...
    if (!shouldRun) {
      ...
      console.log(
        "  AULT_TEST_E2E_LIVE=1 AULT_TEST_PRIVATE_KEY=your_hex_private_key pnpm test:e2e",
      );
